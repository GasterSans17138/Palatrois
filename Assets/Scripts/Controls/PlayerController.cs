...
public sealed class PlayerController : UnitController
{
    ...
    [SerializeField] private bool isSelectionSquadOnClick = false;
    ...
    void StartSelection()
    {
        ...
        // factory selection
        if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, factoryMask, QueryTriggerInteraction.Ignore))
        {
            ...
            PlayerMenuController.HideFormationsMenu();
        }
        // unit selection / unselection
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, unitMask, QueryTriggerInteraction.Ignore))
        {
            ...
            if (selectedUnit != null && selectedUnit.GetTeam() == Team)
            {
                ...
                {
                    SelectUnit(selectedUnit, isSelectionSquadOnClick); // Multiple selections
                }
                else
                {
                    UnselectSelectedSquads();
                    UnselectAllUnits();
                    Debug.Log("Single click");
                    SelectUnit(selectedUnit, isSelectionSquadOnClick); // One selection
                }
                if (SelectedUnitList.Count > 1) PlayerMenuController.ShowFormationsMenu();
                else PlayerMenuController.HideFormationsMenu();
            }
        }
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask, QueryTriggerInteraction.Ignore))
        {
            UnselectCurrentFactory();
            PlayerMenuController.HideFormationsMenu();
    ...
    void UpdateSelection()
    {
        ...
        RaycastHit raycastInfo;
        if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask, QueryTriggerInteraction.Ignore))
        {
    ...
    void EndSelection()
    {
        ...
        List<Unit> current_units = new List<Unit>();
        ...
        foreach (Collider col in colliders)
        {
            ...
            if (selectedEntity.GetTeam() == GetTeam())
            {
                ...
                {
                    current_units.Add(selectedEntity as Unit);
                }
        ...
        {
            if (current_units.Count == 1)
            {
                SelectUnit(current_units[0], true);
            }
            else
            {
                SquadManager.SelectSquads(current_units);

                SelectUnitList(current_units);
            }

            if(SelectedUnitList.Count > 1 && SquadManager.HasLeader(current_units)) PlayerMenuController.ShowFormationsMenu();
            else PlayerMenuController.HideFormationsMenu();
        }
        else
        {
    ...
    protected override void SelectFactory(Factory factory)
    {
        ...
        PlayerMenuController.UpdateFactoryMenu(SelectedFactory, RequestUnitBuild, RequestLeaderBuild, EnterFactoryBuildMode);
    }
    protected override void UnselectCurrentFactory()
    {
        ...
        if (SelectedFactory)
        {
            PlayerMenuController.UnregisterBuildButtons(SelectedFactory.AvailableUnitsCount, SelectedFactory.AvailableLeadersCount, SelectedFactory.AvailableFactoriesCount, SelectedFactory);
        }
    ...
    void ComputeUnitsAction()
    {
        ...
        if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, damageableMask, QueryTriggerInteraction.Ignore))
        {
            ...
            if (other != null)
            {
                if (other.GetTeam() != GetTeam())
                {
                    ...
                    foreach (Unit unit in SelectedUnitList)
                        unit.SetAttackTarget(other, unit.IsInSelectedSquad);
                }
                else if (other.NeedsRepairing())
                {
                    // Direct call to reparing task $$$ to be improved by AI behaviour
                    foreach (Unit unit in SelectedUnitList)
                        unit.SetRepairTarget(other, unit.IsInSelectedSquad);
                }
            }
        }
        // Set capturing target
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, targetMask, QueryTriggerInteraction.Ignore))
        {
            ...
            if (target != null && target.GetTeam() != GetTeam())
            {
                ...
                foreach (Unit unit in SelectedUnitList)
                    unit.SetCaptureTarget(target, unit.IsInSelectedSquad);
            }
        }
