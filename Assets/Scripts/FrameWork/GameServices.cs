...
public class GameServices : MonoBehaviour
{
    ...
    {
        return Instance.CurrentGameData;
    }

    public static UnitController[] GetControllers()
    {
        return Instance.ControllersArray;
    }
    ...
    public static TargetBuilding GetClosestTargetBuilding(Vector3 _position)
    {
        TargetBuilding[] targets = Instance.TargetBuildingArray;
        TargetBuilding target = null;
        float maxDistance = Mathf.Infinity;

        for (int i = 0; i < targets.Length; i++)
        {
            float distance = Vector3.Distance(targets[i].transform.position, _position);
    ...
    void Awake()
    {
        ...
        if (CurrentGameData == null)
            CurrentGameData = GetComponent<GameData>();
